...
{#snippet stationCard(s: Station, stationNumber: number, alliance: 'blue' | 'red')}
	<div
		class="mb-2 rounded border p-2 text-xs transition-colors {s.estop
	...
	>
			...
			<span
				class="font-black {s.estop
					...
					: alliance === 'blue'
						...
						: 'alliance-red-text'}"
			>
				Station {stationNumber} — Team {s.teamNumber || '—'}
			</span>
		...
		<div class="mt-1.5 grid grid-cols-4 gap-1 text-slate-600 dark:text-slate-300">
			...
			<div class="rounded bg-slate-50 px-1.5 py-1 dark:bg-slate-900/60">
				...
				<div
					class="font-semibold {s.missedPackets > 0 ? 'text-yellow-600 dark:text-yellow-400' : ''}"
				>
					{s.missedPackets}
				</div>
			</div>
			<div class="rounded bg-slate-50 px-1.5 py-1 dark:bg-slate-900/60">
				...
				<div
					class="font-semibold {s.secondsSinceLastRobotLink > 3
						? 'text-yellow-600 dark:text-yellow-400'
						: ''}"
				>
		...
		{#if s.wifi}
			<div class="mt-1 grid grid-cols-5 gap-1 text-slate-600 dark:text-slate-300">
				...
				<div class="rounded bg-slate-50 px-1.5 py-1 dark:bg-slate-900/60">
					...
					<div
						class="font-semibold {s.wifi.radioLinked
...
{#snippet fieldDeviceSpecificValuesCell(device: FieldDeviceDiagnostics)}
	<div class="space-y-0.5 text-[11px] text-slate-700 dark:text-slate-300">
		{#each deviceSpecificValues(device) as metric}
			<div>
				<span class="font-semibold text-slate-900 dark:text-slate-100">{metric.label}:</span>
				{metric.value}
			</div>
		{/each}
	</div>
...
<div class="app-neutral-bg min-h-screen text-slate-900 transition-colors dark:text-slate-100">
	...
	<main class="mx-auto flex max-w-[1700px] flex-col gap-3 px-3 py-3">
		...
		>
			<span class="text-sm font-bold text-slate-700 dark:text-slate-200">FTA Status</span>
			<span
		...
		{:else}
			...
			<!-- Field Devices -->
			<div
				class="rounded border border-slate-300 bg-white shadow-sm transition-colors dark:border-slate-700 dark:bg-slate-800"
			>
				<div class="border-b border-slate-200 px-4 py-2 dark:border-slate-700">
					<span class="text-sm font-bold text-slate-700 dark:text-slate-200">Field Devices</span>
					<span class="ml-2 text-xs text-slate-500 dark:text-slate-400"
						>({fieldDevices.length} connected)</span
					>
				</div>
				<div class="p-3">
