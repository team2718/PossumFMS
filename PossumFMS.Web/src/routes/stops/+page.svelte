<script lang="ts">
	...
	const scopeButtonClass = (s: StopScope) =>
		scope === s
			? 'bg-slate-800 dark:bg-slate-100 text-white dark:text-slate-900 border-slate-800 dark:border-slate-100 font-bold'
...
<div class="app-neutral-bg min-h-screen text-slate-900 transition-colors dark:text-slate-100">
	...
	<main class="mx-auto flex max-w-225 flex-col gap-4 px-3 py-4">
		...
		>
			<span class="text-sm font-bold text-slate-700 dark:text-slate-200">Emergency Stops</span>
			<span
		...
		>
			<div
				class="mb-3 text-xs font-bold tracking-wider text-slate-500 uppercase dark:text-slate-400"
			>
				Target Scope
			</div>
			...
			<div class="mt-3 text-xs font-semibold text-slate-600 dark:text-slate-300">
				Current target: <span class="font-bold text-slate-900 dark:text-slate-100"
					>{scopeLabels[scope]}</span
				>
			</div>
		</div>
