...
<div class="p-3">
			...
			{#each logSeverityOptions as level}
				<label class="inline-flex items-center gap-1 text-xs text-slate-700 dark:text-slate-200">
					<input
						...
						class="size-3.5 cursor-pointer rounded border-slate-300 dark:border-slate-600"
					/>
					<span>{level}</span>
				</label>
			{/each}
	...
	<div
		class="max-h-125 overflow-y-auto rounded border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800"
	>
				...
				<tbody class="divide-y divide-slate-100 dark:divide-slate-700">
					{#each filteredLogs as entry (entry.id)}
						<tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50">
							...
							<td
								class="max-w-50 truncate px-2 py-1.5 font-mono text-[10px] text-slate-600 dark:text-slate-400"
							>
								{entry.category}
							</td>
